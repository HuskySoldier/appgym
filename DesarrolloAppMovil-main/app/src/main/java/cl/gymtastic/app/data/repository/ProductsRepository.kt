...
class ProductsRepository(context: Context) {
    ...
    // --- HELPERS (Corrige el error en CartScreen) ---
    suspend fun getNamesById(ids: List<Long>): Map<Long, String> {
        if (ids.isEmpty()) return emptyMap()
        val all = getAll()
        return all.filter { it.id.toLong() in ids }
            .associate { it.id.toLong() to it.nombre }
    }
    ...
    suspend fun getTypesById(ids: List<Long>): Map<Long, String> {
        ...
        val all = getAll()
        return all.filter { it.id.toLong() in ids }
            .associate { it.id.toLong() to it.tipo }
    }

    suspend fun getStockByIds(ids: List<Long>): List<Pair<Long, Int>> { // Ajustado para devolver lista
        if (ids.isEmpty()) return emptyList()
        val all = getAll()
        return all.filter { it.id.toLong() in ids }
            .map { it.id.toLong() to (it.stock ?: 0) }
    }
